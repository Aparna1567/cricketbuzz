  ...
  return (
    <div className={liveMatchStyles.container}>
      ...
      <div className={liveMatchStyles.headerContainer}>
        <div>
          <div className={liveMatchStyles.titleWrapper}>
            ...
            <span className={liveMatchStyles.dotPulse} aria-hidden="true" />
            <span className={liveMatchStyles.dotBase} aria-hidden="true" />
          </div>
      ...
      {loading && matches.length === 0 ? (
        <div className={liveMatchStyles.loadingContainer}><Loader message="Loading live matches..." /></div>
      ) : error ? (
        ...
        <div className={liveMatchStyles.matchesGrid}>
          {matches.map((m) => (
            <div
              ...
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onSelect && onSelect(m.id); }}
              className={liveMatchStyles.matchCard}
            >
              <div className={liveMatchStyles.matchCardInner}>
                <div className={liveMatchStyles.matchHeader}>
                  <div className={liveMatchStyles.matchStatus}>{m.status || 'Match'}</div>
                  <div className={liveMatchStyles.matchTime}>{m.time ? m.time.split(',')[0] : ''}</div>
                </div>
                ...
                <div className={liveMatchStyles.matchFooter}>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={(e) => { e.stopPropagation(); onSelect && onSelect(m.id); }}
        ...
        <div className={liveMatchStyles.noMatchesContainer}>
          <div className="mb-3">No parsed live matches. Raw API for debugging:</div>
          <pre className={liveMatchStyles.rawDataPre}>{JSON.stringify(raw ?? 'No data', null, 2)}</pre>
        </div>
      )}
    </div>
  );
